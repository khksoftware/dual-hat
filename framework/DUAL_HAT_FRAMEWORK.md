...
Where a platform gives one of these obligations code-level enforcement at the response boundary -- a check on the role label, on a nonterminal message's continuation action, or on delegated-worker reconciliation -- that check is a control only while its reach on a live turn equals the obligation it enforces. A sound rule and a green test suite do not establish that; three properties do, and each has been observed missing from a check that had both:

- **A refusal is re-evaluated on retry, never skipped.** A check that avoids refusing forever by passing unconditionally the next time the same turn tries to end has made its own remedy optional: an agent that reads the refusal, changes nothing, and ends the turn again passes cleanly. Bound the loop with a ceiling on refusals per turn instead, counted from the platform's structural record of prior boundaries rather than from the check's own wording, so a reworded message cannot leave the ceiling unreachable. A retry may only narrow the first verdict, on evidence of what the turn did after the refusal; it never adds a refusal of its own.
- **A turn-scoped window closes at every stakeholder message, however the platform records one.** A window closed only by the entries the check recognises as a new turn charges the next turn with the previous turn's findings whenever an interruption is stored in another shape: the finding can never clear, the check's own instruction to correct and continue becomes false, and the check fires hardest on the turns in which the stakeholder spoke most. Derive the boundary from how the platform actually stores an interruption, measured on a real record rather than read from the platform's description of how it presents one, and let a reported finding that is not repeated clear.
- **An act is read from the platform's record of acts, never from text.** A report of completed work and a promise of future work are grammatically identical, and a sentence naming an action is not the action. A check deciding whether a turn did what a message says corroborates the message against the turn's own tool-call records and the dispatch inventory, excluding any payload the agent authored within those acts, since text written beside a promise cannot corroborate it. Where a vocabulary is still needed, use one whose gaps fail loud: a missing word in a list that corroborates an act costs one extra refusal, while a missing word in a list that triggers the check is silence on exactly the case the control exists for.

The residual is stated rather than closed. A remedy the check cannot observe -- a reason given in prose, a decision awaiting the stakeholder -- can only be passed unverified or refused unanswerably, and refusing it reproduces the defect one layer up, so such a pass is reported as unverified and never counted as discharged. Corroboration establishes that an act of the named kind occurred in the turn, not that it was the right act or the one the sentence meant. And no such check reaches a delegated worker's own turns, which the supervising session cannot observe from where its check is attached ([Governing Principles](../governance/GOVERNING_PRINCIPLES.md) principle 12); for those, the dispatching brief remains the only carrier.
