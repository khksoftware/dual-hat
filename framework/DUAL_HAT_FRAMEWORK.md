...
Validation binds the complete candidate tree, environment, dependency state, and profile. A successful capability produces machine evidence, a human conformance review, a clean publication state, a current handover, and a self-contained exit report without waiting to be asked. The report states what changed, what did not, validation results, publication identity, remaining boundaries, operator action, and the exact next gate.
