...
The canonical local product location is `release/v<version>/` within the standalone Dual Hat repository, outside an adopting product repository. The ZIP, release manifest, and checksum stay together there as ignored generated products. Version directories are excluded from source-export, drift, and package-input classification so a release cannot ingest itself or masquerade as source publication content.
