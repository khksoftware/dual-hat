...
class ReleasePackageTests(unittest.TestCase):
    ...
    def test_production_provenance_requires_exact_marker_branch_upstream_and_remote(self) -> None:
        ...
        with TemporaryDirectory() as temporary:
            ...
            responses={("rev-parse","HEAD"):"b"*40,("status","--porcelain=v1","--","."):"",("branch","--show-current"):"main",("rev-parse","--abbrev-ref","--symbolic-full-name","@{upstream}"):"origin/main",("rev-parse","origin/main"):"b"*40,("remote","get-url","origin"):"https://token@example.invalid/approved/dual-hat.git",("remote","get-url","--push","origin"):"git@example.invalid:approved/dual-hat.git",("ls-remote","--heads","origin","refs/heads/main"):"%s\trefs/heads/main"%("b"*40),("rev-parse","HEAD^{tree}"):"c"*40}
            committed={"commit":"b"*40,"manifest_sha256":marker["manifest_sha256"]}
            with patch.object(release_package,"ROOT",root), patch.object(release_package,"_git",side_effect=lambda *args:responses[args]), patch.object(release_package,"verify_commit_tree",return_value=committed):
    ...
    def test_expected_remote_identity_reaches_production_release_validation(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            verified.assert_called_once_with("example.invalid/approved/dual-hat")
