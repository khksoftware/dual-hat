...
def release_provenance_record(
    ...
    *,
    ...
    publication_commit = publication_commit or head
    if not __import__("re").fullmatch(r"[0-9a-f]{40}",publication_commit):
...
def validate_release_set(output: Path, *, source_commit: str | None = None,
    ...
    if require_publication_provenance:
        derived=release_provenance_record(
            expected_remote_identity,
            publication_commit=str(manifest.get("external_publication_commit", "")),
        )
