...
**Governance — derived artifacts.** An artifact produced mechanically from another artifact is a derived artifact, and repository governance now governs the population rather than the individual file. A derived-artifact registry declares, for each canonical record, every derived site and the generation method that emits the bytes, and every obligation is stated against that registry. Derived artifacts of one record are written together or not at all, with a whole-set state stamp where atomicity is unavailable so a partial write fails closed. A partition across derived views is declared once, in the canonical record, rather than living in each renderer. A governed write re-derives and diffs against durable storage in the same act that reports success, because a derivation that would change something on a fresh call immediately after the write is a forgotten derived site. The load-bearing requirement is that a runner computes, names and asserts the entries it does not itself re-derive, and that no statement of coverage is maintained by hand anywhere it appears — the failure being prevented is not a mechanism drifting from reality but a mechanism's prose drifting from the mechanism in one place while another stays correct. Every derived site is enumerated by observing what the writer actually wrote, not from recollection, because the site an author forgets is characteristically not a file but a pointer the record keeps about itself. A canonical record with derived artifacts has a governed writer that validates before rendering and refuses rather than half-applying; an adopter below that floor declares it per entry rather than omitting the entry.

**Governance — bounded write-back.** A projection a person can also type into is the same contract with one axis added, and it is folded into repository governance rather than given a document of its own. The repository is canonical and an external tracker is a derived artifact. Inbound write-back is an allow-list and never a deny-list, because a canonical record gains fields over time and a deny-list's default degrades silently with age while an allow-list's degrades toward refusal. Status, closure and scope never flow inward: the moment a tracker can close an item there are two answers to whether it is done. A field any generation method derives is prohibited inbound independently of the allow-list. Where the two sides disagree the canonical record wins and the overwritten value is reported as a named durable event, because a reconciliation that cannot report what it discarded has not reconciled. The framework defines what a projection with bounded write-back is and does not know any particular tracker exists.

**Governance — the two questions an artifact registry answers.** Validating a registry asks whether the registered artifacts are internally consistent; the obligation that a new active artifact must register asks whether anything is unregistered. These are different questions, and the gap is invisible precisely because the implemented check passes: a registry with zero unregistered artifacts and a registry that cannot see unregistered artifacts produce identical output. A repository carries a detector for the second question or states that its classification claim covers only what was already declared. The predicate is structural — tracked, under a governed path, not matching an archive or evidence convention, absent from the registry — and must not become a filename whitelist, which needs editing whenever a legitimate artifact appears and fails open by default. False positives are expected and designed for, because a detector required to be right the first time is one nobody arms.

**Governance — role transitions.** An Engineering or delegated-worker STOP terminates only that executor's authority. The supervising office immediately preserves the checkpoint, discharges the worker, and continues every safe read-only, design, adjudication or successor-dispatch action in the same turn; it stops only for an explicit stakeholder pause, a named architecture-level hard stop, or a genuine need for new authority or external state change. Saying that control returns to the supervising office and then ending the turn is the defect this clarification forbids, because that office is already the actor responsible for the return.

**Governance — principle 12's residual.** Principle 12 already required an adopter arming the role-label hook to pin the installed artifact against its tracked source. That is now stated as necessary and not sufficient: a hook is a control only once its tracked source exists, a deployed copy matches it, and the platform's own configuration references that copy so something actually invokes it. The third condition is the one nothing checks, and neither a pinning check nor the hook's own tests can answer it — tests import the module and call it directly, which proves the logic and not the wiring, and that is how a hook can be fully tested and entirely inert at the same time. Measured: a hook tracked, deployed, byte-identical, covered by passing tests, and referenced by nothing fired on no turn ever, while its drift check reported clean on every commit. Arming is a third act with its own evidence, and a hook's status is reported as three separate conditions rather than collapsed into one.

**Planning and validation.** Every test plan and cost projection now receives at least two adversarial reduction passes, and further challenge while any generic contingency, duplicated coverage or handoff, hour-scale allowance for a seconds-scale command, or unsupported component remains. The surviving plan records its unreduced starting population, each removal, the measured runtime or command basis for every retained component, the de-duplicated proof set, the distinct risk each retained member owns, its nonclaims, and a hard ceiling that unused allowance cannot enlarge. A challenged estimate returns to architecture rather than being spent. No mandatory safety, refusal, rollback, identity, durability or recovery evidence may be reduced by this.

**Schemas.** A new derived-artifact-registry schema carries the registry entry, with a worked example beside it. Every field is required, including the ones whose honest value is a declared absence, because an absent field and a declared absence are different claims and the registry must distinguish them. It is a new schema rather than fields added to artifact classification, whose closed property set is a deliberate constraint on every adopter and is not widened for the benefit of adopters who have derived artifacts. The worked example deliberately carries three different declared writer maturities, because an example showing only the best case teaches an adopter to omit the entries the registry exists to surface.

**Conformance.** The stale-current-state-projection closure failure is bound to the derived-artifact registry, so a dimension that was a judgement with nothing to evaluate it against becomes computable: every registry entry a closure's changes reach is re-derived and diffed, and the entries the runner did not re-derive are reported by name. A closure naming no registry reports the projection as unchecked rather than claiming the dimension.

**Tooling.** `validate_staged`'s `unknown_staged` arm now applies the same preserved-path filter its sibling arm two lines above already applied, so a caller that correctly declares a preserved path is no longer refused for it. The command-line surface had the mirror of that gap: all three actions called their function with no predicate against a default that rejects everything, so the documented command sequence could not succeed against the repository shape the guide says to run it from; the three actions now pass the framework's own standalone-ownership predicate by default, and a genuinely unknown file is still refused. `stage_manifest_owned`'s worktree-hygiene computation is extracted into a pure set-based rule and a read-only real-worktree check, and `release_package`'s branch-and-upstream provenance computation into a standalone explicitly-rooted function, so both preconditions can be proven before the first write instead of only after several commits have already landed. All four are behavior-preserving for existing callers.

**Release policy.** A production publication requires the active repository's HEAD to be on a local branch named `main` with an upstream tracking ref resolving to `origin/main`; a detached HEAD, a differently named branch, or a branch without that upstream blocks the release, evaluated before any write. This was already enforced in code and stated in neither governing document. A requirement enforced only in code is an emergent property rather than a rule.
