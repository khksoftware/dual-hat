...
- enumerates the publication worktree from git's own view of it -- tracked paths plus untracked paths git itself is not told to disregard -- rather than a raw filesystem walk bounded only by `.git`;
- removes and reports only recognized contained Python bytecode/cache residue before staging, then rejects unowned files, missing governed files, and any remaining cache or common build/tool-cache content, among the tracked and untracked-not-ignored paths that enumeration reaches; content a repository's own ignore rules place outside that view is outside the gate's reach entirely -- neither scanned nor rejected on this basis, so its absence from a refusal proves nothing about it unless it is tracked or untracked-and-not-ignored;
...
If staging raises partway through -- after some paths are already staged but before the run completes -- the index is left staged, and staging refuses to run again until it is empty: an empty index is this gate's own precondition. Inspect what is staged (`git status --short`, `git diff --cached --name-status`) before clearing it, since that is the only record of what the failed run reached and what an operator needs before deciding whether to retry; then unstage with `git reset`, never `git reset --hard`, which would also discard any propagated content the failed run already wrote into the worktree.
...
Review the reported staged paths before committing. After the transparent commit, run committed-tree verification against the exact commit and review its file count and identity before pushing. Do not push if the committed tree contains an unowned, missing, forbidden, hash-mismatched, or secret-bearing file. Push by naming the verified remote and an explicit refspec -- `git push origin HEAD:main` -- never a bare `git push`: a bare push is routed by `remote.pushDefault` or `branch.main.pushRemote` when either is configured, and neither is read or refused by the endpoint verification above. After pushing, fetch and verify local/remote alignment and a clean worktree.
