...
Before governed execution, the selected profile performs a capability preflight against the sealed work order and applicable core. It declares supported mandatory capabilities, permissions and tools, degradation, unavailable services, environment limitations, version compatibility, uncertainty, and the identity and environment fingerprint of the current project-local model-tier mapping. Preflight verifies every tier assigned by the work order and rejects a missing, stale, unavailable, unconfirmed, or fingerprint-mismatched mandatory selection. A profile may add stronger safeguards but may never redefine a mandatory core rule as optional.
